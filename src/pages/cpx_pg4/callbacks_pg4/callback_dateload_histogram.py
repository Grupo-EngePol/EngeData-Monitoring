...
def get_callback_histogram():
    @callback(
        Output("histograms-graph4", "figure"),
        Input("histograms-mean4", "value"),
        Input("histograms-std4", "value"),
    )
...
def get_callback_figtemplate():
    @callback(
        Output("histograms-graph4", "figure",allow_duplicate=True),
...
def get_callback_logotemplate():
    @callback(
        Output('row_logosidebar4', "children",allow_duplicate=True),
        Input("color-mode-switch_pg4", "value"),
        prevent_initial_call=True
    )
